...
import { ChevronRight, Zap, Trophy, Clock } from 'lucide-react';
...
const HomePage = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true);
                const [resNew, resBest, resCat] = await Promise.all([
                    axios.get('/v1/api/products?isNewArrival=true&limit=4'),
                    axios.get('/v1/api/products?isBestSeller=true&limit=4'),
        ...
        <main className="pb-20">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {categories.map((cat, idx) => (
                        <motion.div 
                            ...
                            className="glass p-6 rounded-2xl flex flex-col items-center gap-3 cursor-pointer group hover:bg-primary/10 border border-white/5 hover:border-primary/30 transition-all"
                        >
            ...
            <section className="container mx-auto px-6 mt-24">
                <div className="flex items-center justify-between mb-10">
                    ...
                    <button className="flex items-center gap-2 text-secondary hover:gap-3 transition-all font-medium">
                        View All <ChevronRight className="w-4 h-4" />
                    </button>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            ...
            <section className="container mx-auto px-6 mt-32">
                <div className="glass-card p-10 md:p-16 relative overflow-hidden flex flex-col md:flex-row items-center gap-10">
                    ...
                    <div className="flex-1 relative z-10">
                        <h2 className="text-4xl font-bold text-white mb-4">Join the Cyber Revolution</h2>
                        <p className="text-gray-400 max-w-md">Subscribe to get notified about product launches and exclusive member discounts.</p>
                    </div>
                    ...
                    <div className="flex-1 w-full relative z-10">
                        <div className="flex gap-2 p-2 bg-white/5 rounded-2xl border border-white/10">
                            <input 
                                ...
                                className="flex-grow bg-transparent border-none focus:ring-0 px-4 text-white"
                            />
